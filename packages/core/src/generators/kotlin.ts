...
const typesMap: { [key: string]: string | undefined } = {
  Int: 'int',
};

const getType = (text: string) => typesMap[text] || text;
export function toKotlin(code: string, options = {}) {
...
const types = {
  ...
  Parameter(node: ts.ParameterDeclaration) {
    return `${node.name.getText()}${
      node.type ? `: ${getType(node.type.getText())}` : ''
    }`;
  },
  FunctionDeclaration(node: ts.FunctionDeclaration): string {
    return dedent`fun ${node.name?.escapedText}(${node.parameters
      .map((param) => this.Parameter(param))
      .join(', ')})${node.type ? `: ${getType(node.type.getText())}` : ''} {
